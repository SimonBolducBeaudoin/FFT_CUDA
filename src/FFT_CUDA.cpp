...
void rFFT_Block_CUDA(int n, int size, float *in, std::complex<float> *out) {
  ...
  long long int onembed[] = {(size / 2 + 1) *
                             batch};  // Length of output dimensions
  ...
  if (cufftMakePlanMany64(plan, rank, length, inembed, stride, idist, onembed,
                          stride, odist, CUFFT_R2C, batch,
                          worksize) != CUFFT_SUCCESS) {
    cufftDestroy(plan);
