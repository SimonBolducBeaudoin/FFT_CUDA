...
FFT_CUDA_py(py::array_t<std::complex<DataType>, py::array::c_style> py_in) {
  ...
  return py::array_t<std::complex<DataType>, py::array::c_style>(
      {n}, {2 * sizeof(DataType)}, out, free_when_done);
}

template <class DataType>
py::array_t<std::complex<DataType>, py::array::c_style>
rFFT_CUDA_py(py::array_t<DataType, py::array::c_style> py_in) {
  py::buffer_info buf_in = py_in.request();
  ...
  py::capsule free_when_done(out, free);
  return py::array_t<std::complex<DataType>, py::array::c_style>(
      {n / 2 + 1}, {sizeof(std::complex<DataType>)}, out, free_when_done);
}

template <class DataType>
py::array_t<std::complex<DataType>, py::array::c_style>
rFFT_Block_CUDA_py(py::array_t<DataType, py::array::c_style> py_in, int l_fft) {
  py::buffer_info buf_in = py_in.request();
  ...
  std::complex<DataType> *out;
  out = (std::complex<DataType> *)malloc(batch * (l_fft / 2 + 1) *
  ...
  cudaMalloc((void **)&gpu_out,
             batch * (l_fft / 2 + 1) * sizeof(std::complex<DataType>));
  ...
  return py::array_t<std::complex<DataType>, py::array::c_style>(
      {batch, l_fft},
      {(l_fft / 2 + 1) * sizeof(std::complex<DataType>),
       sizeof(std::complex<DataType>)},
      out, free_when_done);
}
